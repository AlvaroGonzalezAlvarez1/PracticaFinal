    ...
    public Graph<Posicion,Integer> crearGrafoTransitable(Jugador jugador){
        ...
        for(int y=0;y<celdas.length;y++){
            for(int x=0;x<celdas[y].length;x++){
                if(jugador.puedeEntrar(celdas[y][x]) && hayEnemigo(x,y)){
                    Posicion actual=new Posicion(x,y);
                    grafo.addNode(actual);
                    // DERECHA
                    if(x+1 < celdas[y].length && jugador.puedeEntrar(celdas[y][x+1]) && hayEnemigo(x+1,y)){
                    ...
                    // ABAJO
                    if(y+1 < celdas.length && jugador.puedeEntrar(celdas[y+1][x]) && hayEnemigo(x,y+1)){
